...
* *data:* a dataframe where the first column is the independent variable, [S], and the reamining columns (as many as experiment replicates) correspond to the dependent variable, v. 
...
Obtaining the kinetic parameters for the enzyme ADH using non-linear least squares is as eassy as typing:

```{r}
dir.MM(data)
```

As this figure shown, the collected data are less than perfect, but this is a rather common (realitic) situation in the teaching lab (see Figure 1A from *J. Chem. Educ.* 2016, 93, 5, 963-970).
